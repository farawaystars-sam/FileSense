...
def create_prompt(file_summaries):
    ...
    {{
        "proposed_struct": {{
            "Project Planning": [
                ["/full/path/to/doc.pdf", "Project Planning/doc.pdf"],
                ["/full/path/to/screenshot.png", "Project Planning/screeshot.png"]
            ],
            "Technical Documentation": [
                ["/full/path/to/notes.txt", "Technical Documentation/notes.txt"]
            ]
        }},
...
def get_folder_structure(file_summaries):
    ...
    try:
        ...
        # Extract the proposed structure and convert to desired format
        proposed_struct = response_data.get('proposed_struct', {})
        print(f"Proposed structure: {proposed_struct}")
        
        Convert to the new format with semantic categories
        output_structure = {
            "proposed_structure": {
                category: {
                    "files": [file_path for file_path, _ in files]
                }
                for category, files in proposed_struct.items()
            }
        }
        #     output_structure = {
        #     "proposed_structure": {
        #         category: [file_path for file_path, _ in files]
        #         for category, files in proposed_struct.items()
        #     }
        # }
...
if __name__ == "__main__":
    dir_path = "C:\\Users\\Student\\FilESss\\DATASET\\10MB"  # Your test directory
